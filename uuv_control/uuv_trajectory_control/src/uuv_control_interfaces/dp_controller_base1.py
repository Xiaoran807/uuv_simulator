...
from tf_quaternion.transformations import euler_from_quaternion, \
    ...
    quaternion_inverse, quaternion_from_euler
from uuv_control_msgs.msg import Trajectory, TrajectoryPoint, Matrix6, Vector6, TrajectoryPointObjectFollow
    ...
    * `reset_controller` (*service:* `uuv_control_msgs/ResetController`): Reset all 
    ...
    def __init__(self, is_model_based=False, list_odometry_callbacks=None,
        ...
        self._error_pub = rospy.Publisher('error',
                                          TrajectoryPointObjectFollow, queue_size=1)
	...
	self._vel_pub=rospy.Publisher('vel', Vector6, queue_size=10)
        self._count=0
        self._imu_topic_sub=rospy.Subscriber('/rexrov2/imu', Imu, self.imuCallback)
    ...
    def get_list_of_controllers():
        ...
        return [controller.__name__ for controller in
                DPControllerBase1.__subclasses__()]
    ...
    def objectCallback(self, data):
        ...
        ref_position=np.hstack((self.x_wg.data, self.y_wg.data, self.object_position.z))
        self.ref_boxVelocityLinear1=(ref_position-self.ref_boxPosition_prev)/max(0.01,dt)
    ...
    def _update_reference(self):
        ...
        if reference is not None:
           ...
           # self._reference['vel'] = np.hstack((reference.v, reference.w))
            ...
            self._reference['vel'] = np.hstack((self.ref_boxVelocityLinear1, self.ref_boxVelocityAngular))
