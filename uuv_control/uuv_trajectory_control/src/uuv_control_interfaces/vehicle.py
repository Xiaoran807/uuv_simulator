...
class Vehicle(object):
    ...
    def _update_restoring(self, q=None, use_sname=False):
        ...
        """
        if use_sname:
            self.Fg = np.array([0, 0, -self._mass * self._gravity])
            self.Fb = np.array([0, 0, self._volume * self._gravity * self._density])
        else:
            self.Fg = np.array([0, 0, self._mass * self._gravity])
            self.Fb = np.array([0, 0, -self._volume * self._gravity * self._density])
        if q is not None:
        ...
        self._g[0:3] = -1 * np.dot(rotItoB, self.Fg + self.Fb)
        self._g[3:6] = -1 * np.dot(rotItoB,
                                   np.cross(self._cog, self.Fg) + np.cross(self._cob, self.Fb))        
    ...
    def compute_force(self, acc=None, vel=None, with_restoring=True, use_sname=True):
        ...
        if vel is not None:
            ...
            self.nu = self.to_SNAME(self._vel)

        self._update_damping(self.nu)
        self._update_coriolis(self.nu)
        ...
        f = np.dot(self._Mtotal, nu_dot) + np.dot(self._C, self.nu) + \
            np.dot(self._D, self.nu) + g
